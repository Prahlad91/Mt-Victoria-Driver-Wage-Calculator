...
| CL | Carer's leave | Cl. 30.7(b)(ix) | `max(r_hrs, 8h)` at ordinary rate — no OT split (v3.38) |
| AL | Annual leave | Cl. 30.1/30.2 | 8 hrs ordinary + **7.92 hrs** × 20% loading (shiftworker) — loading on 7.92 hrs not 8 (v3.47) |
| PHNW | PH not worked / Time Credit / Picnic day claimed (v3.49) | Cl. 31.7 | 8 hrs ordinary |
...
| PDWP | Picnic Day Worked and Paid (new v3.48) | Cl. 32.1 | Full shift pay at applicable rates + additional 8 hrs ordinary. **$0 if not worked** (no actual times entered). |
...
> **Note — Picnic Day (v3.48/v3.49):** The old `PD` (Picnic day, paid 8 hrs ordinary regardless of working) has been **removed**. It is replaced by two options: `PDWP` for drivers who worked and are owed their shift + bonus day; and `PHNW` (relabelled "PH not worked / Time Credit / Picnic day claimed") for drivers who did not work and are claiming the 8-hr time credit.
...
interface DayState {
  ...
  // Other
  wobod: boolean;
  leaveCat: string;          // 'none' | 'SL' | 'CL' | 'AL' | 'PHNW' | 'PHW' | 'PHWA' | 'PDWP' | 'BL' | 'JD' | 'RDO' | 'LWOP'
...
| 3.54 | June 2026 | **Live preview now reflects cross-midnight split (v3.54).** `FortnightContext.tsx` was calling `previewDay(d, config, codes, unassocAmt, assocChart)` without passing `nextPh` or `nextDow`, so `cmSplit` evaluated to `false` for every day and the entire effective window was assessed at sign-on-day rates. Fixed by changing `days.map(d => ...)` to `days.map((d, i) => ...)` and passing `days[i+1]?.ph ?? false` and `days[i+1]?.dow ?? null`. The live preview now correctly splits a cross-midnight shift at 00:00 and shows next-day (including PH) rates for the post-midnight portion, matching the backend behaviour introduced in v3.53. Note: shift penalties (Cl. 134.1 night/early/afternoon) are **not payable on Saturday or Sunday per Cl. 134.3(a)** — this is correct existing behaviour, not a bug. |
| 3.53 | June 2026 | **Cross-midnight split (v3.53).** When a shift crosses midnight (`cm=True`) and the effective window extends beyond 00:00, the pay window is now split at midnight. Hours before midnight are assessed at the sign-on day's rates (day type, PH status, weekend/weekday). Hours from 00:00 onwards are assessed at the next day's rates. The 8-hour ordinary cap is maintained across the split (e.g. if 8h ordinary is exhausted before midnight, all post-midnight hours are OT). For shifts where post-midnight hours fall on a public holiday **and** are also OT (beyond the 8h ordinary window), the PH loading and OT rate **stack** (option d): OT-tier-1 × B + PH-loading × B = combined stacked rate (e.g. weekday PH OT tier 1 = 1.5 + 0.5 = 2.0× base; tier 2 = 2.0 + 0.5 = 2.5× base). Cl. 78.3 OT tier split (3h boundary) is applied independently per segment; the post-midnight OT counter starts fresh at 0. Sign-on day shift penalties (Cl. 134.1 night/early/afternoon) continue to be assessed from actual sign-on time — they are not applied to the post-midnight segment. Implementation: `compute_day()` in `calculator.py` accepts two new optional parameters `next_ph: bool` and `next_dow: Optional[int]`; `compute_fortnight()` Pass 1 now passes next-day context to each `compute_day()` call. Mirrored in `previewDay()` in `calcPreview.ts`. |
| 3.52 | June 2026 | **PHW/PHWA: OT split beyond 8h ordinary (v3.52).** The 150% PH loading under Cl. 31.5(a) applies only to the **ordinary 8 hours** of a PH worked shift. Hours beyond 8h are overtime and pay at standard Cl. 78.3 OT rates (1.5× for the first 3h OT, 2.0× beyond). Previously, `_compute_leave()` for both `PHW` and `PHWA` applied `actual_hrs × B × 1.5` to all hours — underpaying by up to `(ot2_hrs × B × 0.5)` for shifts exceeding 11h. Fix: split actual hours into `ord_h = min(pay_hrs, 8.0)` and `ot_h = max(0, pay_hrs − 8.0)`, apply 150% loading to `ord_h` only, and add 1026/1110 OT components for `ot_h`. For shifts ≤ 11h (OT entirely within the first 3h tier which is also 1.5×), the total dollar amount is unchanged; the fix only affects shifts > 11h where the 2.0× tier kicks in. Mirrored in `previewLeave()` in `calcPreview.ts`. 58 backend tests pass. |
| 3.51 | June 2026 | **Three bug fixes: auto-suppress threshold, WOBOD 1454 build-up, and PHW/PHWA actual hours (v3.51).** (1) **Auto-suppress threshold lowered from 50% to 25%** — the shift-swap suppression in `_resolve_window()` (calculator.py) and `resolveWindow()` (calcPreview.ts) previously fired at `overlap_ratio < 0.5`. A real lift-up scenario (sched 12:50–20:50, actual 08:00–16:00, 40% overlap — driver started and finished earlier) was incorrectly suppressed. Threshold lowered to `< 0.25` (clear swap = less than one quarter of the shorter shift overlapping); 25–50% overlap is treated as a genuine shifted-start lift-up and allowed through. The canonical Mar 28 auto-suppress case (12% overlap) is unaffected — it remains suppressed under the new threshold. (2) **1454 Assoc Wrk Time (Mileage) now computed for WOBOD days** — the 1454 build-up block in `compute_day()` is never reached for WOBOD days because `compute_day()` returns the `__WOBOD_PENDING__` sentinel early. The formula (`max(0, un-assoc + assoc-payment + dist-pay − sched_hrs)`) is now duplicated in `compute_fortnight()` Pass 2 immediately after the 1496 >10h check, before the sentinel flag is replaced. `sched_hrs` uses `day.r_hrs` if available, otherwise `wobod_hrs`. Mirrored in `calcPreview.ts` WOBOD block before the `return`. (3) **PHW and PHWA pay on actual worked hours, not rostered hours** — `_compute_leave()` for both `PHW` and `PHWA` previously used `r_hrs` (rostered hours from the schedule). When a driver signs on earlier than scheduled (lift-up), the actual duration exceeds `r_hrs` and the 150% loading was under-calculated. Now computes `pay_hrs = r2_hrs((a_e - a_s) / 60)` from `day.a_start`/`day.a_end` with cross-midnight handling (`a_e += 1440` when `day.cm or a_e <= a_s`), falling back to `r_hrs` when no actual times are entered. Mirrored in `previewLeave()` in calcPreview.ts using `toMins`/`toHrs`. 58 backend tests still pass. |
| 3.50 | June 2026 | **Assoc/un-assoc build-up always at base rate (v3.50).** The 1454 "Assoc Wrk Time (Mileage)" component was previously multiplied by the day's rate (`1.5×` on Saturday, `2.0×` on Sunday). Per Cl. 157.1(b), the build-up is always paid at the **base/ordinary rate** regardless of whether the shift falls on a Saturday, Sunday, or public holiday. One-line fix in both `backend/calculator.py` (`b_rate = B`, was `B × (1.5 if is_sat else 2.0 if is_sun else 1.0)`) and `frontend/src/utils/calcPreview.ts` (`const bRate = B`, was `B × (isSat ? 1.5 : isSun ? 2 : 1)`). 58 backend tests still pass (the canonical $7,336.55 fortnight has no Saturday/Sunday 1454 build-up days). |
| 3.49 | June 2026 | **PHNW leave label updated to "PH not worked / Time Credit / Picnic day claimed" (v3.49).** The `PHNW` leave-category dropdown label in `eaRules.ts` has been updated from `'PH not worked / time credit'` to `'PH not worked / Time Credit / Picnic day claimed'` to reflect that this option is now the correct selection for drivers claiming the 8-hr time credit on a Picnic Day they did not work. Calculation unchanged (8 hrs at ordinary rate, Cl. 31.7). Component display name on the Results and Daily Entry pages remains `'PHNW / TC'` (set in v3.46). No backend changes. |
| 3.48 | June 2026 | **New leave category `PDWP` — Picnic Day Worked and Paid (v3.48).** Drivers who work on the annual Picnic Day are entitled to their full shift pay at applicable rates **plus** an additional 8-hr ordinary day payment (Cl. 32.1). Old `PD` leave category (paid 8 hrs ordinary regardless of whether worked) has been removed. Replacement logic: select `PDWP` when worked — shift is calculated normally (ordinary time, OT split, shift penalties, assoc/un-assoc build-up, code 1496 if > 10 hrs, etc.) with a `'Picnic Day — additional 8 hrs'` line appended (`8.00 hrs × base_rate`). If no actual times are entered (driver not worked), `PDWP` returns **$0** — no payment whatsoever. Implementation: `compute_day()` skips `_compute_leave()` for `PDWP` (falls through to normal shift calc); after all components are assembled, appends the extra 8-hr line when `worked_hrs > 0`. Frontend `previewDay()` mirrored identically. `PD` removed from `LEAVE_CATS` in `eaRules.ts`, from `_compute_leave()` in `calculator.py`, and from the leave map in `calcPreview.ts`. 58 backend tests still pass. |
| 3.47 | June 2026 | **Annual leave loading calculated on 7.92 hrs (v3.47).** Per the EA, the 20% leave loading for shiftworkers is applied to **7.92 hours**, not 8.00 hours. Ordinary time payment continues to use 8.00 hours. Per-day total = `r2(8.00 × B) + r2(7.92 × B × 0.20)`. At current rate ($49.81842/hr): ordinary = $398.55, loading = $79.07, total = $477.62 per AL day (was $478.26 with 8-hr loading). Updated in `backend/calculator.py::_compute_leave` and mirrored in `frontend/src/utils/calcPreview.ts::previewLeave`. The loading component now displays `'7.92 hrs'` in the hrs column of the breakdown table. No impact to the canonical $7,336.55 regression (no AL days in that fortnight). 58 backend tests still pass. |
| 3.46 | June 2026 | **PHNW/TC display label + fortnight date auto-population via cycle walk (v3.46).** Two changes: (1) **"PHNW / TC" display label** — the component name emitted on the Results tab and the live pay-preview in the Daily Entry tab for a Public Holiday Not Worked day previously read `'Public Holiday Paid'` (weekend) or `'Public holiday paid'` (weekday). Both backend (`backend/calculator.py::_compute_leave`) and frontend mirror (`frontend/src/utils/calcPreview.ts::previewLeave`) now emit `'PHNW / TC'` regardless of weekend/weekday. The associated flag string is updated to match: `'PHNW / TC: 8 hrs ordinary (Cl. 31.7).'`. (2) **`computeCurrentFortnight()` function** — the master roster's `fn_start` field is the **annual cycle anchor** (e.g. `2025-08-10`), not the specific current-fortnight date; the Setup tab was previously displaying that old anchor date verbatim, which users didn't recognise. The new `computeCurrentFortnight(annualStart)` function counts elapsed days from the anchor to today, takes `floor(days / 14)` fortnights, and returns `anchor + (index × 14 days)` as the ISO date string — e.g. anchor `2025-08-10`, today `2026-06-06` → 300 days → index 21 → returns `2026-05-31`. Applied in both the `useState` initialiser (on mount) and the `useEffect` (when a roster uploads mid-session). Fortnight-roster `fn_start` is still used directly (it is already a specific fortnight date). When no roster has been uploaded the field defaults to `''` (blank) instead of the previous hardcoded `'2025-08-10'`. No backend changes; frontend build clean; 58 backend tests still pass. |
| 3.45 | June 2026 | **Code 1496 — Exp More Than 10 Hours (Cl. 143.5 / Sch.4B Item 12) (v3.45).** When a driver's actual shift duration is **> 10 hours and ≤ 16 hours**, a flat payment of **$14.55** is made under Clause 143.5 and Item 12 of Schedule 4B. Payroll code: `1496`. This applies to **all shift types** including WOBOD. Implementation: (a) `backend/config.yaml` — `shift_penalties.exp_over_10h: 14.55`. (b) `backend/models.py` — `RateConfig.exp_over_10h_rate: float = 14.55`; `PayrollCodes.exp_over_10h: str = '1496'`. (c) `backend/calculator.py` — inserted `if 10.0 < actual_hrs <= 16.0` check in `compute_day()` (non-WOBOD path) and in the WOBOD Pass 2 loop in `compute_fortnight()`. (d) `frontend/src/utils/calcPreview.ts` — mirrored in `previewDay()`. (e) `frontend/src/types/index.ts` — `RateConfig.exp_over_10h_rate?: number`; `PayrollCodes.exp_over_10h?: string`. Two backend regression tests updated: `test_3hrs_ot_all_at_tier1` (11-hr shift → $14.55 added) and `test_4hrs_ot_splits_at_3` (12-hr shift → $14.55 added); both now assert `any(c.code == "1496" ...)`. 58 backend tests pass. |
| 3.44 | June 2026 | **Fortnight start date auto-population from master roster + NSW Public Holiday auto-fill (v3.44).** Two setup-tab UX improvements: (1) **Fortnight start date auto-population** — the `Fortnight start date` field in the Setup tab's "Load roster line" section now automatically populates from the uploaded master roster's `fn_start` field (master roster wins over fortnight roster). Previously defaulted to a hardcoded `'2025-08-10'`. A `useEffect` fires when any upload's `fn_start` changes; if the user has not manually edited the field, it sets the date. Manual edits are tracked via a `dateUserEdited` flag so the auto-fill never overwrites a user-typed date. (2) **NSW Public Holidays auto-fill** — a new `frontend/src/utils/nswPublicHolidays.ts` module ships a hardcoded list of NSW public holidays for 2025–2027 (Bank Holiday excluded — not observed by Sydney Trains). `getPhsForFortnight(fnStart)` filters to the 14-day window and returns matching entries with name and date. Whenever the fortnight start date changes, a second `useEffect` auto-fills the public-holiday chips from the NSW list, preserving any manually-added extras beyond what the list provides. PH chips now show the holiday name alongside the date (e.g., `📆 Fri 3 Apr 2026 — Good Friday ×`). Label below the chip list reads `"Public holidays auto-filled from NSW Gov list"`. No backend changes; frontend build clean. |
| 3.43 | June 2026 | **Admin-only EA reference columns and flag chips in Results tab (v3.43).** Non-admin drivers using the app do not need to see EA clause citations or internal calculation debug flags. `ResultsTab.tsx` now gates three things behind `const isAdmin = !!ctx.adminPassword`: (a) the `EA ref` column in the payslip-format breakdown table (both `<th>` and all `<td>` cells); (b) the `EA` column in the per-day detail table; (c) the yellow flag chips block at the bottom of each day's section (chips such as "Daily OT", "layback clause", etc.). Column `colSpan` on the total row is adjusted to match (6 for admin, 5 for driver). Admin view is unchanged — all columns and chips remain visible. No backend changes; no calculator changes. |
| 3.42 | June 2026 | **24-hour time input via custom `TimeInput` component (v3.42).** The scheduled and actual start/end time fields in `DayRow.tsx` previously used `<input type="time">` which Chrome on macOS with a 12-hour OS locale renders with am/pm regardless of any `lang` or `step` attribute — the browser hard-ignores them. Replaced with a custom `TimeInput` component: `type="text"`, `inputMode="numeric"`, `placeholder="HH:MM"`, `maxLength={5}`. Auto-inserts the colon after the 2nd character is typed (unless the user is deleting); strips all non-numeric non-colon input; truncates to 5 characters. Input event type is inspected to detect backspace/delete so the colon is not re-inserted while the user is clearing the field. This gives a clean 24-hour HH:MM entry experience on all platforms without relying on browser locale behaviour. No backend changes; no calculator changes. |
| 3.41 | June 2026 | **`DEFAULT_ASSOC_CHART` diagram 3155 `unAssocMins` corrected from 50 to 0 (v3.41).** The v3.24 transcription of the 19-Apr-2026 depot chart incorrectly set `unAssocMins: 50` for diagram 3155. The physical chart shows 0 un-assoc minutes and 0 assoc-payment minutes for 3155. With the wrong value, every shift on diagram 3155 was generating a spurious un-associated duties component worth ~0.83h × base rate = ~$41.35. `CACHE_SCHEMA_VERSION` bumped from `'3.40'` to `'3.41'` so any user's localStorage copy of the bad chart is invalidated on first page-load and replaced with the corrected built-in default. No backend changes; 57 backend tests still pass (the canonical $7,336.55 regression test does not involve `unAssocMins` because the test payload uses zero defaults). |
| 3.40 | May 2026 | **Parser-output preview tables (v3.40).** After a successful upload of the master roster, fortnight roster, weekday schedule, or weekend schedule, a collapsible "▼ Show parsed data" panel appears below the upload card group in the admin view. (Drivers see a preview for their own fortnight roster upload.) **Roster preview** — a 14-column day-matrix table (one row per parsed line), showing diagram code and start–end times per cell. Cells with empty diagram strings (parser gaps) are highlighted red with a `?` flag so the admin immediately sees where parsing failed. ADO cells are amber; OFF cells are grey. The line-number column is sticky on horizontal scroll. For fortnight rosters with crew names, a `Crew` column is shown. Day-column headers are labelled `Su 22`, `Mo 23` … when `fn_start` is available, or `D1`–`D14` for the master roster. **Schedule preview** — a 7-column table (Diagram | Type | Sign on | Sign off | Hours | KM | CM) sorted by diagram number, with monospace time cells and 🌙 cross-midnight indicator. Both panels are collapsed by default (zero impact on users who don't need them) and toggle open/closed independently. Both `RosterPreviewTable` and `SchedulePreviewTable` are self-contained components appended to `SetupTab.tsx`; no new files, no backend changes. Frontend build clean. |
| 3.39 | May 2026 | **Master roster parser — diagram one-day-shift bug fix (v3.39).** The `_parse_day_entries()` function in `backend/parsers.py` contained a misattribution bug that caused every diagram number to be assigned to the *next* day's time range instead of its own, producing a visible one-day offset in all loaded roster lines (e.g. line 16: day 2 had empty diagram, day 3 got 3155 instead of 3156, day 14 got 3154 instead of 3651). Root cause: the MTVIC master roster PDF text uses an **inline interleaved format** where each day's data reads `HH:MM – HH:MM 3155 HH:MM – HH:MM 3156 …` (diagram *after* its own time range). When the Pass-1 diagram-collection loop encountered a 4-digit number followed by a time (e.g. `3155 03:20`), it broke out of the loop *without consuming `3155`*, which then became `pending_diag` for the *following* time range. Fix: track `had_pending_diag = pending_diag is not None` before entering the inner diagram-collection loop. When `had_pending_diag is False` (Format A — inline master roster, diagram appears after its time range), consume the 4-digit token into `diag_parts` before breaking, attributing it correctly to the current entry. When `had_pending_diag is True` (Format B — single-row roster with diagram *preceding* its time range, e.g. `3154 02:27 – 10:48`), preserve the original behaviour (break without consuming, let outer loop handle it as pending_diag for the next entry). Second fix in the same function: when no W-suffix hours token is present after a time range, compute `r_hrs` from the start/end times instead of defaulting to `8.0`. This corrects the `sched_hrs` column in the DB (previously always `8.0` for inline-format master rosters). No frontend changes, no API changes. |
| 3.38 | May 2026 | **Excel assoc chart upload + carer's leave fix (v3.38).** Two changes: (1) **Excel (.xlsx) upload for the assoc/un-assoc chart** — `parse_assoc_chart_file()` in `parsers.py` now accepts `.xlsx` files via a new `_parse_chart_xlsx()` helper. Column detection is header-name-aware: searches every sheet for a row containing "diagram" (or "dgm"/"diag") and at least one of the time-column keywords (un-assoc, assoc pmt/payment, assoc calc, build up), then maps columns by name. Handles H:MM text strings, Excel TIME serial numbers (float 0–1 fraction of 1440 min), `datetime.time` objects (from time-formatted cells), and plain integer minutes. Falls back to positional parsing (columns 1–4 after the first 3xxx column) when no named headers are found. The SetupTab file-input `accept` attribute is updated to include `.xlsx`. Drivers/admins can now upload the raw chart workbook exactly as received from the depot, without any pre-conversion. (2) **Carer's leave pays `max(r_hrs, 8h)` at ordinary rate** (matching sick leave rule from v3.30). Previous code paid `r_hrs` at ordinary rate, which under-paid on short shifts (<8 h). Applied in both `backend/calculator.py::_compute_leave` and the mirrored `frontend/src/utils/calcPreview.ts::previewLeave`. PRD §5.9 updated. No OT split. |
| 3.37 | May 2026 | **Assoc chart upload reliability fix (v3.37).** Two bugs prevented a newly-uploaded assoc/un-assoc chart from being applied correctly. (1) **`loadAssocChartCsv` filter bug** — the filter condition `if (unMin > 0 \|\| assMin > 0 \|\| (buildMin ?? 0) > 0)` silently dropped any chart row where all three of `unAssocMins`, `assocPaymentMins`, and `buildUpMins` were zero even when `assocCalcMins` was non-zero (e.g. diagrams 3165, 3167, 3169, 3651, 3658, 3659 in the DEFAULT chart). Fixed by adding `\|\| (calcMin ?? 0) > 0` to the condition. (2) **Unnecessary CSV roundtrip in SetupTab upload handler** — after a successful `POST /api/admin/upload-chart`, the handler was converting the parsed response back to a 5-column CSV string and routing it through `loadAssocChartCsv`, which applied the filter bug and discarded entries. Fixed by adding a new context function `loadAssocChartDirect(chart: AssocChart)` that accepts the parsed `AssocChart` object directly (no CSV conversion, no filter) and writes straight to state + localStorage. The SetupTab upload handler now calls `loadAssocChartDirect(data.chart)` instead of the CSV roundtrip. Both paths (`loadAssocChartCsv` for manual CSV text-area paste, `loadAssocChartDirect` for server upload) now work correctly. No backend changes. Frontend build clean. |
| 3.36 | May 2026 | **LoginScreen — left/right split layout (v3.36).** Restructures the v3.35 top→bottom vertical stack into a side-by-side split: left panel (55% on desktop, 48% on tablet) = navy hero with Sydney Trains branding, 2×2 feature grid, trust badges, disclaimer footer; right panel (flex-1) = light-grey background with the sign-in form card centred vertically. Left/right divide marked by a 3px `${ORANGE}` right-border on the left panel. Responsive: `flex-direction` flips to `column` at ≤768px — left panel becomes a compact header band (logo + wordmark + feature grid, descriptions hidden) with a 3px orange bottom border replacing the right border; right panel flows below. All Sydney Trains brand tokens, NSW DS inputs, Public Sans font, and accessibility attributes from v3.35 are preserved unchanged. PRD §13 updated.
| 3.35 | May 2026 | **LoginScreen — Sydney Trains brand redesign, fully responsive (v3.35).** Replaces the v3.34 split-panel with an authentic Transport for NSW / Sydney Trains visual identity. Brand colours sourced from TfNSW Open Data portal and NSW Digital Design System: train orange `#F6891F`, NSW navy `#002664`, text `#22272B`, border `#CDD3D6`, focus `#0085B3`, error `#B81237`. (1) **Top nav bar** (56px, navy) — Sydney Trains T-logo SVG (orange square, white T), "Sydney Trains" wordmark, "Blue Mountains Line · Mt Victoria Depot" sub-line, EA 2025 badge. (2) **3px orange accent bar** below the nav — mirrors the official TfNSW brand stripe. (3) **Hero section** (navy) — large `clamp`-scaled headline, horizontal-line texture (train-timetable aesthetic), 4-column feature-card grid (responsive: 4-col ≥1024px, auto-fit ≥600px, 2-col mobile, 1-col very small). (4) **Form card** — white, `1px solid #CDD3D6` border, 6px border-radius, 4px orange left-border on card header, NSW DS rectangular inputs (`1.5px solid #CDD3D6`, `border-radius: 4px`, focus ring `0 0 0 3px rgba(0,133,179,.2)`), orange primary button, error banner with left red border, "Don't have access?" help box, OR divider, admin ghost button. (5) **Footer** — dark `#22272B`, disclaimer that this is not an official TfNSW service. (6) **Font** — `Public Sans` (Google Fonts import, official NSW Government typeface) with `Helvetica Neue / Arial` fallback. (7) **Fully responsive** via CSS class breakpoints: ≤600px compacts nav, collapses hero padding, hides feature descriptions; ≤400px switches to 1-column feature grid; ≥1024px stretches to 4-column. (8) **Accessibility**: `aria-invalid`, `aria-describedby` on input, `role="alert"` on error, `role="navigation"`, `role="main"`, `role="img"` on logo SVG, `prefers-reduced-motion` supported. No backend changes.
| 3.34 | May 2026 | **LoginScreen premium redesign — split-panel landing (v3.34).** The sign-in screen is rebuilt as a full-viewport split-panel layout matching Apple/Google design quality. (1) **Left hero panel** (52%, hidden on mobile ≤780px) — deep navy gradient background (`#060f1e` → `#0d1f40`) with radial glow orbs and a subtle dot-grid texture; displays the 🚂 logo mark in a frosted-glass tile, bold two-line headline, org subtitle, 4-item feature list with styled check icons, and trust-badge pills (EA 2025 Compliant, Secure Login, Mt Victoria Depot). (2) **Right form panel** (fluid, white, hairline left separator) — staggered `lsFadeUp` CSS-keyframe entry animations; large monospace employee-ID input with an 8-dot progress indicator below it; primary blue Sign in button with hover lift + glow shadow; inline error alert with icon; a "First time?" help card; and an Admin sign-in ghost button. Responsive: hero panel hides at ≤780px and the form occupies the full viewport on mobile. `prefers-reduced-motion` disables all keyframes. No backend changes. No API changes. Frontend build clean. |
